...
let huamnCount = humanOfficers.count
// Q1: What percentage of humans are female?
let humanFemalecount = femaleOfficers.filter{humanOfficers.contains($0)}.count

let fhuamnCount = Float(huamnCount)
let fhumanFemalecount = Float(humanFemalecount)
let percentage:Float = fhumanFemalecount / fhuamnCount * 100

// Q2: How many first officers are human?
let humanFirstcount = humanOfficers.filter{femaleOfficers.contains($0)}.count

// Q3: Get a list of females sorted by name
let FemaleList = femaleOfficers.sorted()
/*:
...
let CaptainsName: [Officer] = allCaptains.sorted()
print(CaptainsName.map{$0.name})
/*:
Group 2
*/
// Q6: Create an array of officers serving as first officers who are also Vulcan

var array: [String: String] = firstOfficers.filter{ vulcanOfficers.contains($0.key) }
let firstVulcan: [Officer] = array.map{Officer(name: $0, ship: $1)}
print(firstVulcan.map{$0.name})

// Q7: How many these Vulcan first officers have names shorter than 5 characters?

let vulcanCount = firstVulcan.filter{$0.name.count < 5}.count
print(vulcanCount)
